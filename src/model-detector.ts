...
export async function detectModelFiles(
  basePath: string
...
async function findModelWeightFiles(
  basePath: string
): Promise<ModelFileInfo[]> {
  const models: ModelFileInfo[] = []
...
function shouldSkipSmallFile(
  name: string,
  sizeMB: number
): boolean {
  const smallFileExtensions = ['.bin', '.pt', '.pth']
  return (
    smallFileExtensions.some((ext) => name.endsWith(ext)) &&
    sizeMB < MIN_MODEL_SIZE_MB
  )
}
...
async function tryParseHuggingFaceConfig(
  configPath: string
): Promise<ModelFileInfo | null> {
  try {
